...
func TestHistory(t *testing.T) {
	...
	var keyList = make([]utils.DBBKeyFull, batchSize)
	offset := 0x100000 // This is some offset in some file external to the HistoryFile
	start := time.Now()
	var last uint64
	var cnt int

	// Create a utils.DBBKeyFull value for every numKeys
