...
import (
	"github.com/AccumulateNetwork/BlockchainDB/database/history"
...
type KFile struct {
	...
	File            *BFile               // Key File
	History         *history.HistoryFile // The History Database (nil if history is disabled)
	Cache           map[[32]byte]*utils.DBBKey // Cache of DBBKey Offsets
	...
	BloomFilter     *utils.Bloom         // Bloom filter for quick key existence checks
}
...
func OpenKFile(directory string) (kFile *KFile, err error) {
	...
	kFile.Cache = make(map[[32]byte]*utils.DBBKey)
	kFile.BlocksCached = 0

	// History file will be opened separately if needed
...
func NewKFile(
	enableHistory bool, // Controls whether values are immutable (true) or mutable (false)
	directory string,
	offsetCnt uint64,
	keyLimit uint64,
	...
	if enableHistory {
		if kFile.History, err = history.NewHistoryFile(offsetCnt, directory); err != nil {
			return nil, err
		}
	}
	...
	kFile.BloomFilter = utils.NewBloomFilter(10.0, 3) // 10MB Bloom filter with 3 hash functions
...
func (k *KFile) PushHistory() (err error) {
	...
	if len(keyList) > 0 {
		...
		buff := make([]byte, len(keyList)*utils.DBKeyFullSize)
		buffPtr := buff
		for _, key := range keyList {
			...
			buffPtr = buffPtr[utils.DBKeyFullSize:]
		}
...
func (k *KFile) Open() error {
	...
	if k.BloomFilter == nil {
		// Using 10MB as a size, which is a good balance between memory usage and false positive rate
		k.BloomFilter = utils.NewBloomFilter(10.0, 3) // 10MB Bloom filter with 3 hash functions
	}
...
func (k *KFile) PopulateBloomFilterFromHistory() error {
	...
	for i := 0; i < int(hf.OffsetCnt); i++ {
		...
		for offset := int64(start); offset < int64(end); offset += int64(len(buffer)) {
			...
			for keyPos := 0; keyPos+utils.DBKeyFullSize <= len(buffer); keyPos += utils.DBKeyFullSize {
				var key [32]byte
...
// look at the history.
func (k *KFile) Get(Key [32]byte) (dbBKey *utils.DBBKey, err error) {
...
// is free for the user to use (i.e. not part of a buffer used
// by the BFile)
func (k *KFile) kGet(Key [32]byte) (dbBKey *utils.DBBKey, err error) {
	...
	var dbKey utils.DBBKey           //          Search the keys by unmarshaling each key as we search
	for len(keys) >= utils.DBKeyFullSize { //          Search all DBBKey entries, note they are not sorted.
		if [32]byte(keys) == Key {
		...
		keys = keys[utils.DBKeyFullSize:] //       Move to the next DBBKey
	}
	return nil, errors.New("not found")
}
...
func (k *KFile) Put(Key [32]byte, dbBKey *utils.DBBKey) (err error) {
...
func (k *KFile) Close() (err error) {
	...
	for _, key := range keyList {                   // Note that this never updates the first offset. That's okay, it doesn't change
		...
		currentOffset += utils.DBKeyFullSize //                      Add the size of the entry for next offset
	}
...
// the key bins.
func (k *KFile) GetKeyList() (keyValues map[[32]byte]*utils.DBBKey, KeyList [][32]byte, err error) {
	...
	numKeys := len(keyEntriesBytes) / utils.DBKeyFullSize
	if numKeys == 0 {
		return nil, nil, err
	}

	// Create a slice to hold all unique keys, giving priority to NewKeys
	keyValues = make(map[[32]byte]*utils.DBBKey)
	for ; len(keyEntriesBytes) > 0; keyEntriesBytes = keyEntriesBytes[utils.DBKeyFullSize:] {
		dbbKey := new(utils.DBBKey)
