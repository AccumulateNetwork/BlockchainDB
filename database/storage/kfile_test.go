...
func TestKFile(t *testing.T) {
	...
	fr := utils.NewFastRandom([]byte{1})
	nextDBBKey := func() (*utils.DBBKey, [32]byte) {
	...
	for i := 0; i < numKeys; i++ {
		if numKeys > 100 && (i+1)%(numKeys/10) == 0 {
			...
			fmt.Printf("write time : %s\n", utils.ComputeTimePerOp(writesPerSec))
			fmt.Printf("read time  : %s\n", utils.ComputeTimePerOp(readsPerSec))
		}
		dbbKey, k := nextDBBKey()
		dbk, err := kf.Get(k)
...
func TestGetKeyList(t *testing.T) {
	...
	fr := utils.NewFastRandom([]byte{0})

	MakeKeys := func() (keyValues map[[32]byte]*utils.DBBKey, keyList [][32]byte) {
		keyValues = make(map[[32]byte]*utils.DBBKey, numKeys)
