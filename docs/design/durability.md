...
| cut tail | a `sealing.dat` on open is a seal the crash caught between its cut and its link. If the same file already has a segment name, the crash fell between the link and the removal of the staging name: the segment is an orphan above everything named and is adopted, and the staging name is dropped so its records are not replayed as well. Otherwise the file's records are the live tail's, older than anything in `live.dat`, and never fsynced: `recoverSealingTail` builds a new tail from its whole records followed by `live.dat`'s, fsyncs it, renames it over `live.dat`, and only then removes the staging name — the tail exactly as it would have been had the seal never started. Neither input is modified, so a crash mid-fold leaves the next open the same job; a crash after the rename but before the removal leaves `live.dat` beginning with the staged bytes, which the next open recognises and drops the staging name instead of folding twice (`TestInterruptedSealFoldsTheCutTailBack`, `TestInterruptedSealDropsATornCutTail`, `TestInterruptedSealAfterTheLinkIsAdopted`, `TestRecoveryFoldIsIdempotent`). In-process, a seal whose second half fails is resumed by the next seal or by `Close` from wherever it stopped (`TestSealFailureIsResumedByTheNextSeal`) |
